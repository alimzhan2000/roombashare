...
def start(message):
	...
	bot.send_message(message.chat.id, 'Привет👋😊 Я новый бот и зовут меня Roomba (не путать с танцем💃). Я подбираю идеальных тебе соседей👪 для совместной аренды жилья и помогаю людям, которые уже имеют жилье🏠 или его снимают найти себе идеальных клиентов.\n\n Для начала нашей работы выбери что ты хочешь сделать: \n▶Создать объявление, \n▶Просмотреть объявления, \n▶Изменить или просмотреть свое объявление, \n▶Оставить свою обратную связь по моей работе и как ее улучшить', reply_markup=keyboard)
		...
		if u.mode == 1:
			...
			if message.text == 'Другое...':
				...
				if status == 'student':
					...
					keyboard.row('🔙Назад в меню')
					bot.send_message(message.chat.id, 'Какой у тебя режим? Жаворонок - если ты ложишься рано и встаешь рано. \nСова - человек ночи. Любит ложится поздно или вовсе привык работать ночью.', reply_markup=keyboard)
				elif status == 'worker':
